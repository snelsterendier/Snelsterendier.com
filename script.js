...
function betEverything() {
  ...
  if (outcome < 0.2) {
    ...
    alert("🔥 You won! Double chaos, double ranch. REPLACETHISABC screams in the distance.");
  } else {
    balance = 0;
    alert("💀 You lost it all. REPLACETHISABC is laughing at you.");
  }
  localStorage.setItem("snelcoins", balance);
  updateBalance();
}
...
function begForMore() {
  ...
  alert(`🪙 A monster took pity and tossed you ${gift} SnelCoins. Don’t ask where they came from...`);
  localStorage.setItem("snelcoins", balance);
  updateBalance();
}
...
// Cry in the corner
function cryCorner() {
  alert("😭 You cry. Nothing happens. REPLACETHISABC stares at you awkwardly.");
}
